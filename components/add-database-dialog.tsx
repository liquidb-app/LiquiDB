...
export function AddDatabaseDialog({ open, onOpenChange, onAdd }: AddDatabaseDialogProps) {
  ...
  const handleTypeSelect = async (type: DatabaseType) => {
    ...
    if (type === "postgresql") {
      ...
      setIsAutoGenerated(false) // PostgreSQL uses default "postgres" password
    } else if (type === "mysql") {
      setUsername("root")
      setPassword("")
                      ...
                      <Input
                        ...
                        onBlur={async (e) => {
                          ...
                          if (!isNaN(portNum) && portNum > 0) {
                            ...
                            // If port has conflict, try to find next available port as suggestion
                            if (!isValid && portError && portError.includes("in use")) {
                              // Don't auto-change the port, just show the error with suggestion
                              // The user can manually change it if they want
                            }
                          }
                        }}
                    ...
                    <div className="relative">
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          // When user manually types, mark as not auto-generated
                          setIsAutoGenerated(false)
                        }}
                            ...
                            <TooltipTrigger asChild>
                              <div className="flex items-center pl-1 border-l border-border/50 ml-1">
                                <button
                                  type="button"
                                  onClick={() => {
                                    // Generate a new password and overwrite whatever is there
