/**
 * Hook for bulk database operations (start/stop multiple databases)
 */

import { useState, useCallback } from "react"
import { notifySuccess, notifyError, notifyInfo, notifyWarning } from "@/lib/notifications"
import { checkPortConflict } from "@/lib/utils/port-utils"
...
interface UseBulkOperationsProps {
  ...
  startDatabaseRef: React.MutableRefObject<(id: string) => Promise<void>>
}
...
export function useBulkOperations({
  ...
  const toggleDatabaseSelection = useCallback((id: string) => {
  ...
  const handleBulkStart = useCallback(async (databaseIds: string[]) => {
    ...
    console.log(`[Bulk Start] Starting ${stoppedDatabases.length} stopped databases`)
    ...
    const startPromises = validDatabaseIds.map(async (id, index) => {
      try {
        ...
        await startDatabaseRef.current(id)
  ...
  }, [databases, setDatabases, isPortBanned, startDatabaseRef])

  const handleBulkStop = useCallback(async (databaseIds: string[]) => {
    ...
    console.log(`[Bulk Stop] Stopping ${runningDatabases.length} running databases`)
